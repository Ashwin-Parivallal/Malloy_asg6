...
%token<id> NAME
%type<node> atom print_stmt small_stmt opt_test test flow_stmt or_test and_test arith_expr shift_expr and_expr xor_expr expr comparison not_test power factor term opt_yield_test 
%type<node> opt_test_2 pick_yield_expr_testlist star_EQUAL testlist yield_expr plus_COMMA_test opt_COMMA opt_IF_ELSE  pick_yield_expr_testlist_comp testlist_comp
%type<intNumber> pick_PLUS_MINUS pick_multop pick_unop augassign comp_op
%type<node> expr_stmt del_stmt pass_stmt import_stmt global_stmt exec_stmt assert_stmt break_stmt continue_stmt return_stmt raise_stmt yield_stmt import_name import_from suite simple_stmt stmt funcdef compound_stmt  pick_NEWLINE_stmt star_NEWLINE_stmt  if_stmt pick_argument decorator fpdef   argument
%type<vec> plus_stmt star_argument_COMMA opt_arglist arglist star_trailer trailer parameters varargslist star_fpdef_COMMA
...
decorator // Used in: decorators
	: AT dotted_name LPAR opt_arglist RPAR NEWLINE { $$ = NULL; }
	| AT dotted_name NEWLINE { $$ = NULL; }
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist {
			switch($2){
				case PLUSEQUAL:
					$$ = new PlusEqualsBinaryNode($1, $3); 
					pool.add($$);
					break;
				case MINEQUAL:
					$$ = new MinusEqualsBinaryNode($1, $3);
					pool.add($$);
					break;
				case STAREQUAL:
					$$ = new StarEqualsBinaryNode($1, $3);
					pool.add($$);	
					break;
				case SLASHEQUAL:
					$$ = new DivEqualsBinaryNode($1, $3);
					pool.add($$);					
					break;
				case PERCENTEQUAL:
					$$ = new ModEqualsBinaryNode($1, $3);
					pool.add($$);
					break;
	...
	| testlist star_EQUAL  
		{ 
			if( $2) { $$ = new AsgBinaryNode($1, $2); pool.add($$);}
			else $$ = $1;
...
import_name // Used in: import_stmt
	: IMPORT dotted_as_names { $$ = NULL; }
	;
import_from // Used in: import_stmt
	: FROM pick_dotted_name IMPORT pick_STAR_import { $$ = NULL; }
	;
...
global_stmt // Used in: small_stmt
	: GLOBAL NAME star_COMMA_NAME { $$ = NULL; }
	;
...
exec_stmt // Used in: small_stmt
	: EXEC expr IN test opt_COMMA_test { $$ = NULL; }
	| EXEC expr { $$ = NULL; }
	;
assert_stmt // Used in: small_stmt
	: ASSERT test COMMA test { $$ = NULL; }
	| ASSERT test { $$ = NULL; }
	;
...
trailer // Used in: star_trailer
	: LPAR opt_arglist RPAR 
		{
			if($2) $$ = $2;
			else $$ = new std::vector<Node*>(); 
		}
	| LSQB subscriptlist RSQB { $$ = NULL; }
	| DOT NAME { $$ = NULL; }
	;
